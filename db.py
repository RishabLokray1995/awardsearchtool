...
CREATE_SOUTHWEST_TABLE = """
CREATE TABLE IF NOT EXISTS southwest_awards (
...
CREATE_SOUTHWEST_CALENDAR_TABLE = """
CREATE TABLE IF NOT EXISTS southwest_calendar_awards (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    searched_at     TEXT NOT NULL,
    origin          TEXT NOT NULL,
    destination     TEXT NOT NULL,
...
def init_southwest_db(path: str) -> sqlite3.Connection:
    Path(path).parent.mkdir(parents=True, exist_ok=True)
    conn = sqlite3.connect(path)
    conn.execute(CREATE_SOUTHWEST_TABLE)
    conn.execute(CREATE_SOUTHWEST_CALENDAR_TABLE)
    conn.commit()
    return conn
...
def insert_southwest_awards(
    ...
    conn.executemany(
        """
        INSERT INTO southwest_awards
            (searched_at, origin, destination, flight_date, flight_number,
             departure_time, arrival_time, carrier, fare_type, miles,
             taxes_usd, stops, duration_min)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        rows,
    )
...
def insert_southwest_calendar_awards(
    conn: sqlite3.Connection,
    awards: list[dict],
    origin: str,
    destination: str,
    ...
    conn.executemany(
        """
        INSERT INTO southwest_calendar_awards
            (searched_at, origin, destination, flight_date, fare_type, miles, taxes_usd)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        rows,
    )
